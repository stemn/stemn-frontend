...
(function (root, factory) {
  ...
  KicadNewParser.prototype.parseChunk = function (chunk) {
    ...
    this.chunk.split(/[\s]/).forEach((token) => {
      // if ()
    }, this)
  }
  ...
  const alwaysArray = 'fp_text fp_circle fp_line pad'.split(' ')
  ...
  KicadNewParser.prototype.parseLine = function (cmd) {
    ...
    if (cmd.name === 'gr_arc') {
      const angle = parseFloat(cmd.attrs.angle[0]) / 180 * Math.PI
  ...
  KicadNewParser.prototype.parseText = function (cmd, angle) {
    ...
    const effects = cmd.attrs.effects
    cmd.attrs.effects = {}
    ...
    if (cmd.attrs.effects.font && cmd.attrs.effects.font.size) { text.size = parseFloat(cmd.attrs.effects.font.size[0]) }

    const rotate = parseFloat(cmd.attrs.at[2]) || 0
    // TODO:
    ...
    const justify = {};
    (cmd.attrs.effects.justify || []).forEach((justKey) => {
      justify[justKey] = true
    })
    if (justify.mirror) { text.rot = `M${text.rot}` }
    if (justify.right) {
      text.align = 'right'
  ...
  KicadNewParser.prototype.parseVia = function (cmd) {
    cmd.attrs = this.extractAttrs(cmd.args)
    cmd.args  = this.spliceArgs(cmd.args)
    const net = cmd.attrs.net[0]
    const viaDrill = this.netClass[this.netByNumber[net].className].via_drill
    const via = {
      x: parseFloat(cmd.attrs.at[0]),
      y: parseFloat(cmd.attrs.at[1]),
      drill: viaDrill,
  ...
  KicadNewParser.prototype.parseCircle = function (cmd) {
    ...
    const x   = parseFloat(cmd.attrs.center[0])
    const y   = parseFloat(cmd.attrs.center[1])
    const x1  = parseFloat(cmd.attrs.end[0])
    const y1  = parseFloat(cmd.attrs.end[1])
  ...
  KicadNewParser.prototype.parsePad = function (cmd, angle) {
    ...
    const x   = parseFloat(cmd.attrs.at[0])
    const y   = parseFloat(cmd.attrs.at[1])
    // what the f*** ???
    const rot = `R${(parseFloat(cmd.attrs.at[2]) || 0) - (angle || 0)}`
    const w   = parseFloat(cmd.attrs.size[0])
    const h   = parseFloat(cmd.attrs.size[1])

    let name  = cmd.args[0]
    let type  = cmd.args[1]
    let shape = cmd.args[2]
  ...
  KicadNewParser.prototype.parseModule = function (cmd) {
    ...
    if (cmd.attrs.fp_line) {
      cmd.attrs.fp_line.forEach(function (lineCmd) {
        const line = this.parseLine({ name: 'fp_line', args: lineCmd })
    ...
    if (cmd.attrs.fp_circle) {
      cmd.attrs.fp_circle.forEach(function (arcCmd) {
        const line = this.parseCircle({ name: 'fp_circle', args: arcCmd })
        line.layer = this.eagleLayer(line.layer).number
        pkg.wires.push(line)
      }, this) 
    }
    ...
    if (cmd.attrs.pad) {
      cmd.attrs.pad.forEach(function (padCmd) {
        const pad = this.parsePad({ name: 'pad', args: padCmd }, rotate)
  ...
  KicadNewParser.prototype.cmdDone = function () {
    ...
    if (this.cmd.name === 'gr_line' || this.cmd.name === 'gr_arc') {
      const line = this.parseLine(this.cmd)
      // console.log (this.cmd, line);
    ...
    if (this.cmd.name === 'segment' || this.cmd.name === 'via') {
      ...
      const netName = this.netByNumber[netNum].name
    ...
    if (this.cmd.name === 'gr_text') {
      const text = this.parseText(this.cmd)
    ...
    if (this.cmd.name === 'module') {
      const el = this.parseModule(this.cmd)
