...
(function (root, factory) {
  ...
  EagleXMLParser.prototype.parse = function (text) {
    ...
    const boardXML = parser.parseFromString(text, 'text/xml')
  ...
  EagleXMLParser.prototype.parseDOM = function (boardXML) {
    ...
    for (let layerIdx = 0; layerIdx < layers.length; layerIdx++) {
      const layerDict = this.parseLayer(layers[layerIdx])
      board.eagleLayersByName[layerDict.name] = layerDict
      board.layersByNumber[layerDict.number]  = layerDict
    }

    board.elements = {}
    const elements = boardXML.getElementsByTagName('element')
    for (let elementIdx = 0; elementIdx < elements.length; elementIdx++) {
      const elemDict = this.parseElement(elements[elementIdx])
      board.elements[elemDict.name] = elemDict
    }

    board.designRules = {}
    // hashmap signal name -> hashmap layer number -> hashmap 'wires'->wires array, 'vias'->vias array
    const rules = boardXML.getElementsByTagName('designrules')
    for (let ruleIdx = 0; ruleIdx < rules.length; ruleIdx++) {
      const rule = rules[ruleIdx]

      const ruleParams = rule.getElementsByTagName('param')
      for (let ruleParamIdx = 0; ruleParamIdx < ruleParams.length; ruleParamIdx++) {
        const ruleParam = ruleParams[ruleParamIdx]
        let ruleParamVal = ruleParam.getAttribute('value')
        var m
        if (m = ruleParamVal.match(/^([\d\.]+)mil$/)) { ruleParamVal = parseFloat(m[1]) * 0.0254 }
    ...
    board.signalItems = {}
    // hashmap signal name -> hashmap layer number -> hashmap 'wires'->wires array, 'vias'->vias array
    const signals = boardXML.getElementsByTagName('signal')
    for (let sigIdx = 0; sigIdx < signals.length; sigIdx++) {
      ...
      const vias = signal.getElementsByTagName('via')
      for (let viaIdx = 0; viaIdx < vias.length; viaIdx++) {
        const viaDict = this.parseVia(vias[viaIdx])
      ...
      const contacts = signal.getElementsByTagName('contactref')
      for (let contactIdx = 0; contactIdx < contacts.length; contactIdx++) {
        const contact = contacts[contactIdx]
        const elemName = contact.getAttribute('element')
        const padName = contact.getAttribute('pad')
        const elem = board.elements[elemName]
        if (elem) elem.padSignals[padName] = name
      }
    }

    board.packagesByName = {}
    const packages = boardXML.getElementsByTagName('package')
    for (let packageIdx = 0; packageIdx < packages.length; packageIdx++) {
      ...
      const descriptionEls = pkg.getElementsByTagName('description')
      if (descriptionEls && descriptionEls.length) { var description = descriptionEls[0].textContent }

      const packageSmds = []
      const smds = pkg.getElementsByTagName('smd')
      for (let smdIdx = 0; smdIdx < smds.length; smdIdx++) {
        const smd = smds[smdIdx]
        packageSmds.push(this.parseSmd(smd))
      }

      const packageRects = []
      const rects = pkg.getElementsByTagName('rectangle')
      ...
      const packagePads = []
      const pads = pkg.getElementsByTagName('pad')
      ...
      const packagePolys = []
      const polys = pkg.getElementsByTagName('polygon')
      ...
      const bbox = this.board.calcBBox(packageWires)
    ...
    board.plainTexts = {}
    board.plainHoles = []
    const plains = boardXML.getElementsByTagName('plain')	// Usually only one
    for (let plainIdx = 0; plainIdx < plains.length; plainIdx++) {
  ...
  EagleXMLParser.prototype.parseSmd = function (smd) {
    let smdX  = parseFloat(smd.getAttribute('x')),
  ...
  EagleXMLParser.prototype.parsePoly = function (poly) {
    const width = parseFloat(poly.getAttribute('width'))
    const vertexes = [];
    [].slice.apply(poly.getElementsByTagName('vertex')).forEach((vertexEl) => {
      vertexes.push({
  ...
  EagleXMLParser.prototype.parsePad = function (pad) {
    const drill = parseFloat(pad.getAttribute('drill'))
    let diameter = parseFloat(pad.getAttribute('diameter'))
    ...
    const padRot = pad.getAttribute('rot') || 'R0'
    return {
  ...
  EagleXMLParser.prototype.parseVia = function (via) {
    ...
    const drill = parseFloat(via.getAttribute('drill'))
    const viaType = 'Outer'
    const dr = this.board.designRules
    const viaMult = parseFloat(dr[`rvVia${viaType}`])
    const viaMax  = parseFloat(dr[`rlMaxVia${viaType}`])
    const viaMin  = parseFloat(dr[`rlMinVia${viaType}`])
    let viaRest = drill * viaMult
    if (viaRest < viaMin) {
      viaRest = viaMin
  ...
  function circleCenter(x1, y1, x2, y2, angle) {
    ...
    let r = Math.abs(d / 2 / Math.sin(angle / 180 / 2 * Math.PI)),
      r0 = r,
      r1 = r
    ...
    if (dAngle1 > -0.0000001 && dAngle1 < 0.0000001) {
      return [cx1, cy1, rad11, r]
    } else if (dAngle2 > -0.0000001 && dAngle2 < 0.0000001) {
      return [cx2, cy2, rad21, r]
    } 
    console.log('something wrong: angle:', angle, 'angle1:', angle1, 'dangle1', (dAngle1 > -0.0000001 || dAngle1 < 0.0000001), 'angle2:', angle2, 'dangle2:', (dAngle2 > -0.0000001 || dAngle2 < 0.0000001))
    return [cx2, cy2, rad21, r]
  ...
  EagleXMLParser.prototype.parseWire = function (wire) {
    let width = parseFloat(wire.getAttribute('width'))
    if (width <= 0.0) width = this.minLineWidth

    const layer = parseInt(wire.getAttribute('layer'))

    let x1 = parseFloat(wire.getAttribute('x1')),
      y1 = parseFloat(wire.getAttribute('y1')),
      x2 = parseFloat(wire.getAttribute('x2')),
      y2 = parseFloat(wire.getAttribute('y2'))

    const style = wire.getAttribute('style')

    const curve = parseInt(wire.getAttribute('curve'))

    if (curve) {
      const center = circleCenter(x1, y1, x2, y2, curve)

      let angle = Math.PI * (curve / 180)
  ...
  EagleXMLParser.prototype.parseText = function (text) {
    ...
    if (!content) content = ''
    const textRot = text.getAttribute('rot') || 'R0'
    let textAlign = text.getAttribute('align') || '',
      align,
      valign,
  ...
  EagleXMLParser.prototype.parseElement = function (elem) {
    let elemRot    = elem.getAttribute('rot') || 'R0',
    ...
    for (let attribIdx = 0; attribIdx < elemAttribs.length; attribIdx++) {
      let elemAttrib = elemAttribs[attribIdx],
        attribDict = {},
      ...
      if (name) {
        ...
        let rot = elemAttrib.getAttribute('rot')
        if (!rot) { rot = 'R0' }
        const attribAngle = this.board.angleForRot(rot)
