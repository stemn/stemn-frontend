...
export default () => {
  ...
  function flip(flipStatus) {
    ...
    const flipArray = wG.isFlipped ? { x: 0, y: 0, z: 1, w: 0 } : { x: 0, y: 0, z: 0, w: 1 }
  ...
  function init(layers, domParent, globalInstances) {
    ...
    wG.layers = layers
    wG.limits = _.reduce(wG.layers, (bound, layer) => ({
    ...
    // Ambient light.
    const ambientLight = new THR51.AmbientLight(0xcccccc)
    wG.scene.add(ambientLight)

    // Sun light.
    if (has3D) {
      const sunLight = new THR51.SpotLight(0xcccccc, 0.3)
      sunLight.position.set(0, 150000, 0)
      wG.scene.add(sunLight)
    }

    // Board.
    const Material = has3D ? THR51.MeshPhongMaterial : THR51.MeshBasicMaterial
    ...
    // Add the sides.
    let boardSides = new THR51.CubeGeometry(wG.boards.width, wG.constants.geometry.boardThickness, wG.boards.height, 1, 1, 1, undefined, { py: 0, ny: 0 })
    const boardMaterial = new Material(wG.constants.materials.board)
    ...
    wG.layers.sort((a, b) => (a.type || 10) - (b.type || 10))
  ...
  function drillHoles(boardMaterial) {
    const holeMaterial = boardMaterial.clone()
    holeMaterial.side = THR51.BackSide
    if (wG.layers[0].isGerber) {
      _.forEach(wG.layers, (layer) => {
        if (layer.type == wG.index.BOARD) {
          _.forEach(layer.cmds, (cmd) => {
            const r    = layer.scale * layer.shapes[cmd[1]][1] / 2
            let hole = new THR51.CylinderGeometry(r, r, wG.constants.geometry.boardThickness, 32, 0, true)
      ...
      _.forEach(wG.layers[0].plainHoles, (cmd) => {
        const r    = cmd.drill / 2
        let hole = new THR51.CylinderGeometry(r, r, wG.constants.geometry.boardThickness, 32, 0, true)
        // hole.computeVertexNormals();
        hole = new THR51.Mesh(hole, holeMaterial)
  ...
  function render() {
    ...
    if (wG.repaint !== null) {
      ...
      if (wG.repaint === 0) {
        ...
        while (wG.repaint <= wG.layers.length && !wG.layers[wG.repaint - 1].enabled) { wG.repaint++ }
  ...
  function makeBoard(w, h, inverted) {
    ...
    const stretch = canvas.width / canvas.height
    if (stretch > 4) { canvas.width-- } else if (stretch < 0.25) { canvas.height-- }
