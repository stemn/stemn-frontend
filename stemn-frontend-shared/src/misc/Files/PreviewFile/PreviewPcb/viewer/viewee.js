...
(function (root, factory) {
  ...
  function scaleToFit(a) {
    const fitElement = this.canvas
    let fitWidth = fitElement.width,
  ...
  function setScale(scale, noResize) {
    ...
    const fitElement = this.scaleToFitSelector ? document.querySelector(this.scaleToFitSelector) : this.canvas
    if (!fitElement) {
      return
    }
    let fitWidth = fitElement.offsetWidth,
      fitHeight = fitElement.offsetHeight

    const scrollX = (fitElement.scrollLeft + fitElement.clientWidth / 2) / fitElement.scrollWidth
    const scrollY = (fitElement.scrollTop + fitElement.clientHeight / 2) / fitElement.scrollHeight
  ...
  ViewEE.prototype.findParser = function (text) {
    let board = {}
    const parserFound = ViewEE.parsers.some(function (parser) {
      if (!parser) return
  ...
  function layerColor(colorIdx) {
    let rgb = this.colorPalette[colorIdx]
    if (!rgb) {
  ...
  function angleForRot(rot) {
    ...
    let spin = (rot.indexOf('S') >= 0), // TODO: spin rotate
  ...
  function matrixForRot(rot) {
    const angle = this.angleForRot(rot)
    let spin = angle.spin, // TODO: spin rotate
  ...
  function mirrorLayer(layerIdx) {
    ...
    let name = this.layersByNumber[layerIdx].name,
  ...
  function max() {
    ...
    return Math.max(...args.filter(val => !isNaN(val)))
  }

  function min() {
    const args = [].slice.call(arguments)
    return Math.min(...args.filter(val => !isNaN(val)))
  }

  function calculateBounds() {
    let minX = ViewEE.LARGE_NUMBER,
    ...
    for (const netName in this.signalItems) {
    ...
    // Elements
    for (const elemKey in this.elements) {
      const elem = this.elements[elemKey]
      const pkg = typeof elem.pkg === 'string' ? this.packagesByName[elem.pkg] : elem.pkg
      const rotMat = elem.matrix
      for (const smdIdx in pkg.smds) {
      ...
      for (const wireIdx in pkg.wires) {
