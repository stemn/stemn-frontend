...
service('SyncUtilService', function ($state, SyncUrlService) {
    ...
    var viewerFileTypes = {
        general       : {
            ...
            autodesk  : ['3dm','3ds','asm','cam360','catpart','catproduct','cgr','collaboration','dae','dgn','dlv3','dwf','dwfx','dwg','dwt','dxf','exp','f3d','fbx','g','gbxml','iam','idw','ifc','ige','iges','igs','ipt','jt','model','neu','nwc','nwd','obj','pdf','prt','rcp','rvt','sab','sat','session','skp','sldasm','sldprt','smb','smt','ste','step','stl','stla','stlb','stp','wire','x_b','x_t','xas','xpr'],
...
service('SyncUrlService', function ($state) {
    ...
    function getChildPath(children){
        if(children){
            return _.map(children, service.getPath).join(',')
        }
    }
            ...
            if ($scope.files) {
                _.forEach($scope.files, function (item) {
                    if (item.name.toLowerCase() == 'readme.md') {
                        ...
                        SyncService.download(item.downloadUrl).then(function (response) {
                            $scope.readme.path = item.path;
