...
(function (root, factory) {
  ...
  function parseFlags(flagString, context) {
    ...
    if (!flagString.match(/0x\d+/i)) {
      flagString.split(',').forEach((flag) => { flags[flag] = true })
      return flags
    }
    const flagInt = parseInt(flagString)
    ...
    if (flagInt & 0x0008 && (context === 'pin' || context === 'via')) { flags.hole = true } // For pins and vias, this flag means that the pin or via is a hole without a copper annulus.
    if (flagInt & 0x0008 && context === 'pad') { flags.nopaste = true } // For pads, set to prevent a solderpaste stencil opening for the pad. Primarily used for pads used as fiducials.
    if (flagInt & 0x0010 && context === 'line') { flags.nopaste = true } // If set for a line, indicates that this line is a rat line instead of a copper trace.
    if (flagInt & 0x0010 && (context === 'pin' || context === 'pad')) { flags.pininpoly = true } // For pins and pads, this flag is used internally to indicate that the pin or pad overlaps a polygon on some layer.
    if (flagInt & 0x0010 && context === 'poly') { flags.clearpoly = true } // For polygons, this flag means that pins and vias will normally clear these polygons (thus, thermals are required for electrical connection). When clear, polygons will solidly connect to pins and vias.
    if (flagInt & 0x0010 && context === 'element') { flags.hidename = true } // For elements, when set the name of the element is hidden.
    if (flagInt & 0x0020 && context === 'element') { flags.showname = true } // For elements, when set the names of pins are shown.
    if (flagInt & 0x0020 && (context === 'line' || context === 'arc')) { flags.clearline = true } // For lines and arcs, the line/arc will clear polygons instead of connecting to them.
    if (flagInt & 0x0020 && context === 'poly') { flags.fullpoly = true } // For polygons, the full polygon is drawn (i.e. all parts instead of only the biggest one).
    if (flagInt & 0x0040) { flags.selected = true } // Set when the object is selected.
    if (flagInt & 0x0080 && (context === 'element' || context === 'pad')) { flags.onsolder = true } // For elements and pads, indicates that they are on the solder/top side.
    if (flagInt & 0x0080 && (context === 'line' || context === 'via')) { flags.auto = true } // For lines and vias, indicates that these were created by the autorouter.
    if (flagInt & 0x0100 && (context === 'pin' || context === 'pad')) { flags.square = true } // For pins and pads, indicates a square (vs round) pin/pad.
    if (flagInt & 0x0200 && context === 'line') { flags.rubberend = true } // For lines, used internally for rubber band moves.
    if (flagInt & 0x0200 && (context === 'pin' || context === 'pad' || context === 'via')) { flags.warn = true } // For pins, vias, and pads, set to indicate a warning.
    /**/if (flagInt & 0x0400 && (context === 'pin' || context === 'via')) { flags.usetherm = true } // Obsolete, indicates that pins/vias should be drawn with thermal fingers.
    /**/if (flagInt & 0x0400 && context === 'line') { flags.silk = true } // Obsolete, old files used this to indicate lines drawn on silk.
    if (flagInt & 0x0800 && (context === 'pin' || context === 'via')) { flags.octagon = true } // For pins and pads, indicates a square (vs round) pin/pad.
    if (flagInt & 0x1000) { flags.drc = true } // Set for objects that fail DRC.
    if (flagInt & 0x2000) { flags.lock = true } // Set for locked objects.
    if (flagInt & 0x4000 && (context === 'pin' || context === 'pad')) { flags.edge2 = true } // For pads, indicates that the second point is closer to the edge. For pins, indicates that the pin is closer to a horizontal edge and thus pinout text should be vertical.
    if (flagInt & 0x8000) { flags.marker = true } // Marker used internally to avoid revisiting an object.
    if (flagInt & 0x10000) { flags.connected = true } // If set, this object has been as physically connected by FindConnection().
  ...
  function parseAttrsXYThicknessNameFlags(cmd) {
    ...
    const thickness = parseNumeric(cmd.args.shift(), !cmd.square)

    const flags = parseFlags(cmd.args.pop())
    const name  = cmd.args.pop()
  ...
  gEDAParser.prototype.parseChunk = function (chunk, idx) {
    if (chunk.charAt(0) === '#' || chunk.trim().length === 0) { return }

    const polyPointRx  = /\[([^\]]+)\s+([^\]]+)\]/
    const polyPointRxG = new RegExp(polyPointRx.source, 'g')

    let m = chunk.match(/^\s*(\w+)(?:\s*\[(.*)\]|\((.*)\))$/)
    if (m) {
      ...
      this.cmd = { name: m[1], args: parseTokens(m[2] || m[3]), children: [], square: !!m[2] }
      ...
      const pointsType = this.cmd._hole ? '_hole' : 'vertexes'

      this.cmd[pointsType] = this.cmd[pointsType] || []

      m.forEach(function (pointCoords) {
        const point = pointCoords.match(polyPointRx)
  ...
  gEDAParser.prototype.parseLine = function (cmd) {
    ...
    if (cmd.name === 'Line') {
      const flags = parseFlags(cmd.args.pop())
      if (cmd.args.length) {
        const clearance = parseNumeric(cmd.args.shift(), !cmd.square)
      }
    }
  ...
  gEDAParser.prototype.parseArc = function (cmd) {
    ...
    const deltaAngle = cmd.args.pop()
    const startAngle = cmd.args.pop()
  ...
  gEDAParser.prototype.parseText = function (cmd, angle) {
    ...
    if (cmd.args.indexOf('hide') > -1) { text.display = 'off' }
  ...
  gEDAParser.prototype.parseVia = function (cmd) {
    ...
    const shape = cmd.attrs.flags.octagon ? 'octagon' : 'circle'
  ...
  gEDAParser.prototype.parsePad = function (cmd) {
    ...
    if (cmd.args.length) {
      const clearance = parseNumeric(cmd.args.shift(), !cmd.square)
      const mask      = parseNumeric(cmd.args.shift(), !cmd.square)
    } else {
      // TODO
      const absoluteCoords = true
    }
  ...
  gEDAParser.prototype.parseElement = function (cmd) {
    ...
    const textFlags = parseFlags(cmd.args.pop())
    const textScale = cmd.args.pop()
    const textDir   = cmd.args.pop()
    ...
    const desc = cmd.args.shift()
    const name = cmd.args.shift()
    ...
    const bbox = this.board.calcBBox(pkg.wires)
    pkg.bbox = bbox
    ...
    cmd.attrs.fp_text.forEach(function (txtCmd) {
      const txt = this.parseText({ name: 'fp_text', args: txtCmd }, rotate)
      txt.type = txtCmd[0]
  ...
  gEDAParser.prototype.cmdDone = function () {
    const contextPath = this.context.map(cmd => cmd.name).join('>')
    // console.log (contextPath, this.cmd);

    // layer groups
    if (this.cmd.name === 'Groups') {
      const groups = this.cmd.args[0].split(':')
      groups.forEach(function (group) {
        const layers = group.split(',')
    ...
    if (this.cmd.name === 'Layer') {
      const layerId   = parseInt(this.cmd.args[0])
      const layerName = this.cmd.args[1]

      const isTopLayer    = layerName === 'solder'    || layerName === 'top'
      const isBottomLayer = layerName === 'component' || layerName === 'bottom'

      let isSignalLayer = false
      ...
      const eagleLayer = this.eagleLayer(layerName.toLowerCase())
      ...
      this.cmd.children.forEach(function (child) {
        if (child.name === '') {
          ...
          const netName = 'undefined'
          // console.log (this.cmd, line, netName);

          if (!this.board.signalItems[netName]) this.board.signalItems[netName] = {}
          const signalLayerItems = this.board.signalItems[netName]


          if (isSignalLayer) {
            if (!signalLayerItems[eagleLayer.number]) { signalLayerItems[eagleLayer.number] = { wires: [], vias: [] } }
            signalLayerItems[eagleLayer.number].wires.push(line)
          } else {
            if (!this.board.plainWires[eagleLayer.number]) { this.board.plainWires[eagleLayer.number] = [] }
      ...
      const signalLayers = []
      this.cmd.args.forEach((layer) => {
        const layerId = parseInt(layer.name)
        if (layer.args[1] === 'signal') { signalLayers[layerId] = true }
      })

      const maxSignalLayer = signalLayers.length - 1

      this.cmd.args.forEach(function (layer) {
        // Layer names can be changed
        // console.log (layer);
        const layerId = parseInt(layer.name)
        const layerName = layer.args[0] // F.Cu, B.Cu and so on
        const layerType = layer.args[1] // user, signal, mixed
        const layerShow = layer.args[2] !== 'hide' // 'hide' or nothin

        let eagleLayer = this.eagleLayer(layerName)
    ...
    if (this.cmd.name === 'net_class') {
      ...
      for (let i = 2; i < this.cmd.args.length; i++) {
        const netCmd = this.cmd.args[i]
    ...
    if (this.cmd.name === 'Via') {
      const via = this.parseVia(this.cmd)
      ...
      if (!signalLayerItems[via.layers]) { signalLayerItems[via.layers] = { wires: [], vias: [] } }
    ...
    if (this.cmd.name === 'segment' || this.cmd.name === 'via') {
      ...
      const netNum = this.cmd.attrs.net
      var netName = this.netByNumber[netNum].name
      // console.log (this.cmd, line, netName);
      if (!this.board.signalItems[netName]) this.board.signalItems[netName] = {}
      var signalLayerItems = this.board.signalItems[netName]
      ...
      if (netNum) {
        if (!signalLayerItems[eagleLayerNumber]) { signalLayerItems[eagleLayerNumber] = { wires: [], vias: [] } }
    ...
    if (this.cmd.name === 'gr_circle') {
      const circle = this.parseCircle(this.cmd)
    ...
    if (this.cmd.name === 'Element') {
      const el = this.parseElement(this.cmd)
  ...
  if (typeof process !== 'undefined') {
    const gp = new gEDAParser({ matrixForRot() {}, calcBBox() {} })
    const fs = require('fs')
