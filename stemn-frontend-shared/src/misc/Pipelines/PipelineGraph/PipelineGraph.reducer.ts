...
export const pipelineGraphReducer = (state: IPipelineGraphStoreState = initialState, action) => {
  switch (action.type) {
    ...
    case 'PIPELINE_DIAGRAM/ADD_STEP':
      return i.assocIn(
        state,
        ['diagrams', action.payload.diagramId, 'model', 'steps', action.payload.stepId],
        action.payload.step,
      )
